...
- 2026-04-27: Per-turn cost estimate in the assistant footer. New `app/src/features/models/pricing.ts` (`estimateTurnCostUsd` / `formatCostUsd` / `formatTokenCount`) computes USD using the catalog's `pricing.standard` rates, applies the long-context multiplier (>272K input tokens) when present, and treats `outputTokens` as already including reasoning tokens. `MessageFooter` renders `~$X.XX` after the model label with a Tooltip showing input/output token counts × effective $/MTok and the long-context note when applicable.
- 2026-04-27: Refreshed `/models` catalog. Added `pricing` (USD per 1M tokens; `standard`/`priority`/`batch`/`longContext`) to `ModelCatalogEntry` on both server (`server/src/modules/models/models.types.ts` + `models.service.ts`) and app (`app/src/features/models/types.ts`); refreshed gpt-5.5 metadata (knowledge cutoff 2025-12-01, full tool/Responses/Realtime support, `access.api: false` per Codex docs) and corrected gpt-5.4-mini context window (200K → 400K) and max output (65K → 128K). Fallback `ModelCatalogEntry` in `server/src/modules/conversations/context.service.ts` now includes `pricing: null`. Pricing payload feeds the upcoming usage cost estimator.
