...
- `src/browser.rs` — child-webview-per-tab browser. `BrowserState` keeps a `HashMap<id, Webview>`; commands `browser_open`/`navigate`/`back`/`forward`/`reload`/`stop`/`set_bounds`/`set_visible`/`close`/`list_alive`/`eval`/`meta_report` and events `browser://navigated`/`title`/`favicon`/`load-state`/`url-report`. Requires `tauri = { features = ["unstable"] }` for `Window::add_child` + `WebviewBuilder`. All tabs share a single profile dir at `~/.agnt/browser-profile/`.
